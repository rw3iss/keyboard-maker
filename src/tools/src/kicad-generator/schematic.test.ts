...
import { generateSchematic } from './schematic.js';
...
describe('generateSchematic', () => {
  ...
  it('outputs valid kicad_sch format', () => {
    const result = generateSchematic(layout4, matrix4, makeConfig());
    expect(result).toContain('kicad_sch');
  ...
  it('contains battery when bluetooth enabled', () => {
    ...
    const result = generateSchematic(layout4, matrix4, config);
    expect(result).toContain('BT1');
    expect(result).toContain('Battery');
  });

  it('does not contain battery when bluetooth disabled', () => {
    const config = makeConfig({
  ...
  it('contains ROW and COL net labels', () => {
    const result = generateSchematic(layout4, matrix4, makeConfig());
  ...
  it('contains symbol instances', () => {
    const result = generateSchematic(layout4, matrix4, makeConfig());
    expect(result).toContain('symbol_instances');
  });
});
