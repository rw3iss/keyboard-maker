...
export type PinFunction = 'gpio' | 'vcc' | 'gnd' | 'vbus' | 'usb_dp' | 'usb_dm' | 'reset' | 'battery' | 'nc';

/** Pin map for bare chips — maps pad numbers to net functions.
 *  GPIO pads are assigned dynamically; only special pads are listed. */
...
/** Full MCU data record loaded from component database */
export interface McuData {
  ...
  formFactor?: string;
  gpioCount?: number;
  gpioPins?: Array<{ pin: string; label: string; analog?: boolean }>;
  hasUsb?: boolean;
  hasBle?: boolean;
  package?: McuPackage;
  pinMap?: McuPinMap;
